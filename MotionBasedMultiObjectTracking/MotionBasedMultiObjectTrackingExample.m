...
% Bounding box gives you [x y width height]

function [coordinates] = MotionBasedMultiObjectTrackingExample(videoName)
...
while ~isDone(obj.reader)
    ...
    % Detect on every frame, the coordinates of the dancer and write them in
    % a matrix.this array will have as many lines as the frames of the video,
    % and two columns, respectively the x coordinate and the y coordinate.
    if ~isempty(centroids) && ~isempty(bboxes)
        
        [x,y] = FilterMultipleObject(centroids,bboxes);
        %s = "x: "+x + "     y: " + y
        coordinates(index,1) = x;
        coordinates(index,2) = y;
        index = index+1;
    end
